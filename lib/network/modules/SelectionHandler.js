...
class SelectionHandler {
  ...
  hoverObject(event, pointer) {
    ...
    if (hoverChanged === true || (object instanceof Node && object.options.shape == 'toast')) {
      if (object && object.pointer) {
        object.pointer = this.canvas.DOMtoCanvas(pointer);//pointer;
      }

      this.body.emitter.emit('_requestRedraw');
      console.log('hoverObject - _requestRedraw');
    }
  }
