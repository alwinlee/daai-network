  ...
  getEdgeAt(pointer, returnEdge = true) {
    ...
    for (let i = 0; i < this.body.edgeIndices.length; i++) {
      ...
      if (edge.connected) {
        ...
        const xMiddle = (edge.from.x + edge.to.x) / 2;
        const yMiddle = (edge.from.y + edge.to.y) / 2;
        const distMiddle = edge.edgeType.getDistanceToEdge(xMiddle - 1, yMiddle - 1, xMiddle + 1, yMiddle + 1, canvasPos.x, canvasPos.y);

        if (distMiddle < mindist) {
          console.log(' hover in middle of edge!')
        }
